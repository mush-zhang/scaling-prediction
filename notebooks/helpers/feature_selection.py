...
#     @staticmethod
#     def __linear_weights_orders(X, y):
...
#         return FeatureSelection.__sparse_argsort(scores)
    ...
    @staticmethod
    def __boruta_orders(X, y, est_name):
    ...
    def __get_feature_orders(self, method, est_name=None, feature_type=None):
        ...
        for i in range(expr_num):
            ...
            if method == 'Lasso':
                orders = FeatureSelection.__lasso_weights_orders(X, y)
            elif method == 'ENet':
                orders = FeatureSelection.__enet_weights_orders(X, y)
            elif method == 'Variance':
                orders = FeatureSelection.__variance_threshold(X, y)
            elif method == 'fANOVA':
                orders = FeatureSelection.__select_k_best(X, y, method='fANOVA')
            elif method == 'Chi2':
                orders = FeatureSelection.__select_k_best(X, y, method='Chi2')
                ...
                orders = FeatureSelection.__boruta_orders(X, y, est_name)
        ...
        final_orders = FeatureSelection.__sparse_argsort(feature_importance)
        # final_orders = FeatureSelection.__all_argsort(feature_importance)
    ...
    def __sfs_orders(self, num_features, direction, est_name, feature_type=None):
        ...
        final_orders = FeatureSelection.__sparse_argsort(feature_importance)[:num_features]
        top_features = [feature_cols[j] for j in final_orders]
        return top_features
